...
- **Designing a new theme**: add a `.theme-<name>:not(.dark) { ... }` + `.dark.theme-<name> { ... }` block at the bottom of `color-theme.css`. Override Base tokens only (typically `--bg-base / --bg-surface / --bg-elevated / --accent-default / --accent-fg / --border-default / --ring-default` — about 10 values). Region tokens auto-derive. Then add the name to `VALID_DARK_THEMES` in `src/lib/settings.ts` and the dropdown in `src/features/settings/panels/appearance.tsx`. Do NOT override locked surfaces in a preset.
